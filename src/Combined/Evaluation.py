...
class correctValCounter():
    ...
    def evalN(self, output_true:torch.Tensor, y:torch.Tensor, type="Soft", offset=0, unknownRejection=True, indistribution = None):
        ...
        if indistribution == None:
            ...
            if offset == 0:
                numclasses = self.classCount+1

        #format inputs
        if y.dim() == 2:
            y_val = torch.argmax(y, dim=1)-offset
            #this checks if none of them are correct. The values of unknowns is the highest+1 instead of zero because I did not think ahead.
            y_outside = self.classCount * (torch.max(y,dim=1)[0] == 0)
            y_val += y_outside

            y_oh = F.one_hot(y_val,self.classCount+1)[:self.classCount]
        else:
            y_val = y-offset
            y_oh = F.one_hot(y_val,self.classCount)

        if output.dim() == 2:
            out_val= torch.argmax(output[:,:len(y_oh[0])], dim=1)
        else:
        ...
        ybincount = torch.bincount(y_val,minlength=self.classCount+1)
        self.count_by_class+=ybincount[:self.classCount]
        self.count_unknown+=ybincount[self.classCount]
        ...
        if unknownRejection:
            ...
            #ok, follow my logic. If out_oh makes no guesses and y_val is in an out of set position. It adds one.
            self.found_unknowns += ((out_oh.sum(dim=1)==0) * (y_val == self.classCount)).sum()
    ...
    def zero(self):
        ...
        self.count_unknown = 0  #how many unknowns exist
        self.found_unknowns = 0 #how many unknowns have been found
    ...
    def openMaxMod(self,percentages:torch.Tensor):
        ...
        for logits in percentages:
            ...
            output_open_new, _ = OpenMaxByMaXu.openmax(self.weibullmodel, list(range(self.classCount)),logits.detach().numpy()[np.newaxis,:], 0.23, alpha=3)
    ...
    def fScoreUnknown(self):
        #Positive predictive Value
        precision = self.found_unknowns/(self.count - self.totals_for_guesses.sum().item())
        #True Positive Rate
        recall = self.found_unknowns/self.count_unknown
        return 2*(precision*recall)/(precision+recall)
