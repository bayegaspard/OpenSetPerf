...
plotter[1] += torch.tensor([x for x in range(25)]).to(torch.int)+1
...
with torch.no_grad():
    ...
    for x in plotter[1]:
        weibullmodel.append(OpenMaxByMaXu.fit_weibull(mavs,distances,catagories,tailsize=int(x.item())))
    ...
    for batch,(X,y) in enumerate(testing):
        ...
        for logits in output_true:
            ...
            output_open_new, output_soft_new = OpenMaxByMaXu.openmax(weibullmodel[10], catagories,logits.numpy()[np.newaxis,:], 0.4)
        ...
        for x in plotter[1]:
            for logits in output_true:
                #this is where the openmax is run, I did not create the openmax
                output_open_new, output_soft_new = OpenMaxByMaXu.openmax(weibullmodel[int(x.item())-1], catagories,logits.numpy()[np.newaxis,:], 0.4)
                plotter[0][int(x.item())-1] += (torch.argmax(torch.tensor(output_open_new), dim=0) != CLASSES).sum()
...
with torch.no_grad():
    ...
    for batch,(X,y) in enumerate(unknowns):
        ...
        for logits in output_true:
            #this is where the openmax is run, I did not create the openmax
            output_open_new, output_soft_new = OpenMaxByMaXu.openmax(weibullmodel[0], catagories,logits.numpy()[np.newaxis,:], 0.4)
            output_soft.append(torch.tensor(output_soft_new).unsqueeze(dim=0))
            output_open.append(torch.tensor(output_open_new).unsqueeze(dim=0))
